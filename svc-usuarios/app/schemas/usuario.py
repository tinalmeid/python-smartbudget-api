...
class UsuarioCreate(BaseModel):
    ...
    """
    email: EmailStr = Field(..., description="Email do usuário", json_schema_extra={"example": EXAMPLE_EMAIL})
    senha: str = Field(min_length=8, max_length=128, json_schema_extra={"example": EXAMPLE_SENHA})
    nome: str | None = Field(None, description="Nome do usuário", json_schema_extra={"example": EXAMPLE_NOME})
...
class UsuarioLogin(BaseModel):
    ...
    """
    email: EmailStr = Field(..., description="Email do usuário", json_schema_extra={"example": EXAMPLE_EMAIL})
    senha: str = Field(..., description="Senha do usuário", json_schema_extra={"example": EXAMPLE_SENHA})
...
class UsuarioResponse(BaseModel):
    ...
    id: str = Field(..., description="UUID do usuário", json_schema_extra={"example": EXAMPLE_ID})
    email: EmailStr = Field(..., description="Email do usuário", json_schema_extra={"example": EXAMPLE_EMAIL})
    nome: str | None = Field(None, description="Nome do usuário", json_schema_extra={"example": EXAMPLE_NOME})
    ativo: bool = Field(..., description="Indica se a conta do usuário está ativa", json_schema_extra={"example": True})
    criado_em: datetime = Field(..., description="Data e hora de criação do registro", json_schema_extra={"example": EXAMPLE_DATA})
...
class TokenResponse(BaseModel):
    ...
    """
    access_token: str = Field(..., description="Token de acesso", json_schema_extra={"example": EXAMPLE_TOKEN})
    refresh_token: str = Field(..., description="Token de refresh", json_schema_extra={"example": EXAMPLE_TOKEN})
